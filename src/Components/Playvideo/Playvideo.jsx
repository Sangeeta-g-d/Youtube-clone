...
const Playvideo = () => {
  ...
  const fetchVideoData = async () =>{
    //fetching video data
    const videoDetaile_url = `https://youtube.googleapis.com/youtube/v3/videos?part=snippet%2CcontentDetails%2Cstatistics&id=${videoId}&key=${API_KEY}`
    await fetch(videoDetaile_url).then(res=>res.json()).then(data => setapiData(data.items[0]));
  }
  useEffect(()=>{
    fetchVideoData();
  },[videoId])
  ...
  return (
    <div className='play-video'>
        ...
        <iframe src={`https://www.youtube.com/embed/${videoId}?autoplay=1`} frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" ></iframe>
            ...
            <h3>{apiData?apiData.snippet.title:"Title here"}
            ...
            <div className="play-video-info">
                ...
                <div>
                    ...
                    <span><img src={dislike}/> {apiData?value_converter(apiData.statistics.dislikeCount):5}</span>
                ...
                {comment.map((item,index)=>{
                    ...
                    <div>
                        ...
                        <div className='comment-action'>
                            ...
                            <img src={value_converter(item.snippet.topLevelComment.snippet.dislikeCount)}/>
                        </div>
                    </div>
                </div>
                    )
